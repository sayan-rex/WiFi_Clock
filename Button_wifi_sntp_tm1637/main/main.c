/*
 * This program allows the user to set up a Wi-Fi connection and fetch the current time
 * from an NTP server using the Simple Network Time Protocol (SNTP). The program displays the time
 * in 12-hour format with AM/PM indicator and also shows the current date. The user can use the
 * buttons to enter the Wi-Fi credentials (SSID and password) and connect to a Wi-Fi network.
 * Once the time and Wi-Fi credentials are set, the program will fetching the time from the NTP server,
 * the time is saved in the RTC and ESP32 c3 module enter into deep sleep to save power. The ESP32 c3
 * will wake up after 60 seconds display updated time in TM1637 display. Wi-Fi credentials is stored in
 * the Non-Volatile Storage (NVS) for future uses.
 */
#include <stdio.h>
#include <string.h>
#include <time.h>
...
void app_main(void)
{
	 // Initialize TM1637 display
	    tm1637_led_t *led = tm1637_init(PIN_CLK, PIN_DIO);
	    tm1637_set_brightness(led, 7); // Set brightness level (0-7)
    ...
    // Display the time on the TM1637 display with the dot segment enabled for the colon
    tm1637_set_segment_number(led, 0, time_str[0] - '0', false);
    tm1637_set_segment_number(led, 1, time_str[1] - '0', true);  // Display the dot segment (":")
    tm1637_set_segment_number(led, 2, time_str[3] - '0', false);
    tm1637_set_segment_number(led, 3, time_str[4] - '0', false);

    // Wait for 5 seconds
    vTaskDelay(pdMS_TO_TICKS(5000));

    // Turn off the display
    tm1637_set_segment_number(led, 0, 0xFF, false);
    tm1637_set_segment_number(led, 1, 0xFF, false);
    tm1637_set_segment_number(led, 2, 0xFF, false);
    tm1637_set_segment_number(led, 3, 0xFF, false);
