...
#define SEMAPHORE_TAKE(port) do { \
        if (!xSemaphoreTake(states[port].lock, pdMS_TO_TICKS(100))) \
        { \
...
esp_err_t i2c_dev_take_mutex(i2c_dev_t *dev)
{
    ...
    if (!xSemaphoreTake(dev->mutex, pdMS_TO_TICKS(100)))
    {
...
esp_err_t i2c_dev_probe(const i2c_dev_t *dev, i2c_dev_type_t operation_type)
{
    ...
    if (res == ESP_OK)
    {
        ...
        res = i2c_master_cmd_begin(dev->port, cmd, pdMS_TO_TICKS(10));
