...
static int time_sync_retry_count = 0;
...
void display_task(void *arg) {
    while (1) {
        ...
        if (buttonLongPress && longPressCount == 2) {
            // If second long press, break out of the loop and terminate the task
            break;
        }
    }

    // Task cleanup or additional actions can be added here if needed
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        if (s_retry_num < ESP_MAXIMUM_RETRY) {
     	   ...
     	   // If connection fails, reset SSID and password in NVS and set longPressCount to 0
...
void app_main(void)
{
    ...
    // Check if connection to the AP failed and time is not set
    if (s_retry_num >= ESP_MAXIMUM_RETRY && sntp_get_sync_status() != SNTP_SYNC_STATUS_COMPLETED) {
        ESP_LOGE(TAG, "Connection to AP failed and time is not set. Resetting...");
